 ...
 */
static int stm_lock(struct mtd_info *mtd, loff_t ofs, uint64_t len)
{
 ...
 * Returns negative on errors, 0 on success.
 */
static int stm_unlock(struct mtd_info *mtd, loff_t ofs, uint64_t len)
{
	struct spi_nor *nor = mtd->priv;
	int status_old, status_new;
	u8 mask = SR_BP2 | SR_BP1 | SR_BP0;
 ...
 * negative on errors.
 */
static int stm_is_locked(struct mtd_info *mtd, loff_t ofs, uint64_t len)
{
	struct spi_nor *nor = mtd->priv;
	int status;
...
int spi_nor_scan(struct spi_nor *nor)
{
	...
	if (JEDEC_MFR(info) == SNOR_MFR_MICRON ||
	    JEDEC_MFR(info) == SNOR_MFR_SST) {
		mtd->_lock = stm_lock;
		mtd->_unlock = stm_unlock;
		mtd->_is_locked = stm_is_locked;
	}
#endif
